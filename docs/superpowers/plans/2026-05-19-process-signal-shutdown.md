...
**Goal:** Add project-level SIGTERM/SIGHUP shutdown handling so LangGraph agent process exits interrupt foreground terminal waits, terminate reference implementation background processes, and clean terminal environments without orphaning subprocess groups.

**Architecture:** Keep terminal process mechanics unchanged: foreground commands already run in their own process group and `_wait_for_process()` already kills that group after `is_interrupted()` becomes true. Add a thin LangChain project lifecycle layer that broadcasts interrupt to every active `terminal_execution_scope()` thread, sleeps for a configurable grace window to let daemon/tool threads observe the interrupt, then raises `KeyboardInterrupt`; an idempotent atexit cleanup kills all registered background processes and cleans all terminal environments.

**Tech Stack:** Python 3.12, LangChain/LangGraph `create_agent`, existing `agent_core.terminal_lifecycle`, terminal toolkit `process_registry`, pytest via `/home/miku/miniforge3/envs/langchain/bin/python`.

---

## Current State

- `agent_tools/terminal_toolkit/environments/local.py` starts foreground shell commands with `preexec_fn=os.setsid` on Unix, so shell children form a separate process group.
- `agent_tools/terminal_toolkit/environments/base.py` already polls every 200ms and calls `_kill_process(proc)` when `is_interrupted()` is true.
- `agent_tools/terminal_toolkit/process_registry.py` already supports `kill_all(task_id=None)` for all running background sessions.
...
- `agent_tools/terminal_toolkit/terminal_tool.py` already registers `cleanup_all_environments` with `atexit`, but it does not kill every running background session before environment cleanup.
...
- Modify `README.md`
  - Document signal behavior, `AGENT_SIGTERM_GRACE`, and direct `agent.invoke(...)` responsibilities.
...
SIGTERM_GRACE_ENV = "AGENT_SIGTERM_GRACE"
...
def run_process_shutdown_cleanup(*, reason: str = "process_exit") -> dict[str, Any]:
    ...
    try:
        ...
        logger.exception("Failed to kill reference implementation background processes during shutdown.")
        errors.append(f"process_registry.kill_all: {exc}")

    try:
        cleaned_environments = cleanup_all_environments()
    except Exception as exc:
        logger.exception("Failed to clean terminal environments during shutdown.")
...
- On SIGTERM or SIGHUP, the handler marks every active `terminal_execution_scope(thread_id)` thread as interrupted, sleeps for `AGENT_SIGTERM_GRACE` seconds, then raises `KeyboardInterrupt` so the hosting runtime can terminate the run.
- `AGENT_SIGTERM_GRACE` defaults to `1.5`. Set it to `0` to skip the grace window, or increase it if the host routinely runs terminal commands with slow shutdown behavior.
...
- Spec coverage: The plan maps the reference implementation's five-step chain to this repository: signal registration in `build_agent()`, interrupt broadcast through `agent_core.terminal_lifecycle`, grace window in `agent_core.process_lifecycle`, `KeyboardInterrupt` propagation, and atexit cleanup that includes both background process registry and terminal environments.
...
- Orphan prevention: Foreground commands remain handled byreference implementation `_wait_for_process()` and `_kill_process()`. Background sessions are covered by `process_registry.kill_all()` during atexit cleanup.
- Placeholder scan: No task contains TBD-style implementation gaps; code snippets and commands are concrete.
- Type consistency: `thread_id` means LangGraph thread id string; `python_thread_id` means `threading.Thread.ident`; `task_id` remains the runtime hashed task id managed by existing session-context helpers.
