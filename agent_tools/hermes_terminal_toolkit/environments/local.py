...
from ..paths import get_subprocess_home
...
def _build_provider_env_blocklist() -> frozenset:
    """Return a conservative blocklist of common secret-bearing env vars."""
    blocked: set[str] = set()
...
def _read_terminal_shell_init_config() -> tuple[list[str], bool]:
    """Return (shell_init_files, auto_source_bashrc) from environment variables."""
    files_raw = os.getenv("TERMINAL_SHELL_INIT_FILES", "")
    files = [item.strip() for item in files_raw.split(os.pathsep) if item.strip()]
    auto_bashrc = os.getenv("TERMINAL_AUTO_SOURCE_BASHRC", "true").lower() in ("true", "1", "yes", "on")
    return files, auto_bashrc
