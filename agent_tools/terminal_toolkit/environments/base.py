"""Base class for all terminal toolkit execution environment backends.
...
# TERMINAL_TOOLKIT_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("TERMINAL_TOOLKIT_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to terminal toolkit home/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_toolkit_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__TERMINAL_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all terminal toolkit backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/terminal-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/terminal-cwd-{self._session_id}.txt"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $__terminal_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"TERMINAL_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via TERMINAL_TOOLKIT_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __TERMINAL_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        from ..command_utils import rewrite_compound_background

        exec_command = rewrite_compound_background(exec_command)
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from ..command_utils import transform_sudo_command
