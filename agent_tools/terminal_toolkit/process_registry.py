...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/terminal_bg_{session.id}.log"
        pid_path = f"{temp_dir}/terminal_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/terminal_bg_{session.id}.exit"
...
def process_tool(args, *, task_id: str | None = None):
    ...
    if action == "list":
        ...
        if not session_id:
            return _tool_error(f"session_id is required for {action}")
        if action == "poll":
    ...
    return _tool_error(f"Unknown process action: {action}. Use: list, poll, log, wait, kill, write, submit, close")
