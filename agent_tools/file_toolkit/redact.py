...
# Snapshot at import time so runtime env mutations cannot enable or disable
# redaction mid-session. OFF by default; opt in with
# `AGENT_REDACT_SECRETS=true` in the process environment.
_REDACT_ENABLED = os.getenv("AGENT_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across agent tools.
    Use it anywhere a secret needs to be shown truncated for
    debuggability while still keeping the bulk hidden.
